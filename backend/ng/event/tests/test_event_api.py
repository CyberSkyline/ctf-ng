...
class Test_Event_Admin_Start:
    def post_endpoint(self, event_id: int) -> str:
        return f"/ng/admin/events/{event_id}/start_event"

    def test_admin_start_locked_event(self, admin_client, event_factory):
        """
        Test that an admin can start a locked event
        """
        event = event_factory(
            name="Locked Event to Start",
            public=True,
            locked=True
        )

        response = admin_client.post(self.post_endpoint(event.id), json={})
    ...
    def test_admin_start_already_started_event(self, admin_client, event_factory):
        """
        Test that starting an already started event returns an error
        """
        event = event_factory(
        ...
        assert data["success"] is False
        assert "errors" in data
        assert "Event has already started" in data["errors"]["business_logic"]

    def test_non_admin_cannot_start_event(self, logged_in_client, event_factory):
        """
        Test that non-admin users cannot start events
        """
        event = event_factory(
            name="Event for Non-Admin Start",
            public=True,
            locked=True
        )

        response = logged_in_client.post(self.post_endpoint(event.id), json={})

        assert response.status_code == 403

    def test_start_nonexistent_event(self, admin_client):
        """
        Test that starting a non-existent event returns 404
        """
        response = admin_client.post(self.post_endpoint(9999), json={})
...
class Test_Event_Admin_End:
    def post_endpoint(self, event_id: int) -> str:
        return f"/ng/admin/events/{event_id}/end_event"

    def test_admin_end_started_event(self, admin_client, event_factory):
        """
        Test that an admin can end a started event
        """
        event = event_factory(
            name="Started Event to End",
            public=True,
            locked=False
        )
    ...
    def test_admin_end_already_ended_event(self, admin_client, event_factory):
        """
        Test that ending an already ended event returns an error
        """
        event = event_factory(
        ...
        assert data["success"] is False
        assert "errors" in data
        assert "Event has already ended" in data["errors"]["business_logic"]

    def test_non_admin_cannot_end_event(self, logged_in_client, event_factory):
        """
        Test that non-admin users cannot end events
        """
        event = event_factory(
            name="Event for Non-Admin End",
            public=True,
            locked=False
        )
