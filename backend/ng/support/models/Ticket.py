...
class Ticket(db.Model):
    ...
    def create_ticket_with_initial_message(
        ...
        """
        Create a new ticket with an initial message in a single transaction.
        This ensures both ticket and message are created atomically.
        ...
        Returns:
            Ticket: The created ticket instance with message
        """
        from .TicketMessage import TicketMessage

        validated_ticket_data = cls.validate(
            {
                "subject": subject,
                "author_id": author_id,
                "event_id": event_id,
        ...
        message_validator = BaseValidator()
        message_validator.validate_string(
            {"text": text},
            "text",
        ...
        validated_message_data = message_validator.validate()

        ticket = cls(
            subject=validated_ticket_data["subject"],
            author_id=validated_ticket_data["author_id"],
            event_id=validated_ticket_data.get("event_id"),
            team_id=validated_ticket_data.get("team_id"),
            challenge_id=validated_ticket_data.get("challenge_id"),
        )

        message = TicketMessage(
            text=validated_message_data["text"],
            author_id=author_id,
        )
