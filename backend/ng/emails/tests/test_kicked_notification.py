...
class TestKickedPlayerEmailNotification:
    ...
    def test_admin_kicks_player_sends_email(
        self, app, db_session, user_factory, admin, event, team_factory, email_config, mock_ses
    ):
        """
    ...
    def test_captain_kicks_player_creates_notification(
        self, app, db_session, user_factory, event, team_factory, email_config, mock_ses
    ):
        """
        ...
        with app.app_context():
            ...
            team = team_factory(event=event, members=[captain, member])
            ...
            notification = Notification.query.filter_by(
                recipient_id=member.ctfd_user.id, type=NotificationType.TEAM_MEMBER_KICKED
            ).first()
