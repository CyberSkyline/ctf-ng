...
class TestEmailSendingSmartRouting:
    ...
    def email_config(self, app):
        ...
        app.config["ADMIN_SUPPORT_INBOX_EMAILS"] = "admin@ctf.com,support@ctf.com"
    ...
    def mock_ses(self):
        ...
        """
        with patch("ng.emails.services.email_sender.boto3.client") as mock_boto_client:
            mock_ses = MagicMock()
    ...
    def test_new_ticket_goes_to_admin_inbox(self, app, db_session, user, event, team_factory, email_config, mock_ses):
        """
        ...
        with app.app_context():
            ...
            create_ticket(subject="Test Issue", text="I need help with something", current_user=user, event_id=event.id)

            mock_ses.send_email.assert_called_once()
            call_args = mock_ses.send_email.call_args[1]

            recipient_emails = call_args["Destination"]["ToAddresses"]
            assert set(recipient_emails) == {"admin@ctf.com", "support@ctf.com"}
            assert "New Support Ticket: Test Issue" in call_args["Message"]["Subject"]["Data"]

    def test_admin_reply_to_team_ticket_notifies_team_members(
        self, app, db_session, user, admin, event, team_factory, user_factory, email_config, mock_ses
    ):
        """
        ...
        with app.app_context():
            ...
            team = team_factory(event=event, members=[user, user2, user3])

            ticket = create_ticket(
                subject="Team Issue", text="Team needs help", current_user=user, event_id=event.id, team_id=team.id
            )
            mock_ses.reset_mock()

            create_ticket_message(text="I'll help you with this", author_id=admin.id, ticket=ticket, is_admin=True)

            mock_ses.send_email.assert_called_once()
            call_args = mock_ses.send_email.call_args[1]

            recipient_emails = set(call_args["Destination"]["ToAddresses"])
            expected_emails = {user.email, user2.ctfd_user.email, user3.ctfd_user.email}
            assert recipient_emails == expected_emails
            assert "Admin Reply" in call_args["Message"]["Body"]["Html"]["Data"]

    def test_admin_reply_to_non_team_ticket_notifies_author(
        self, app, db_session, user, admin, ticket_factory, email_config, mock_ses
    ):
        """
        Test that admin reply to non team ticket goes to ticket author
        """
        with app.app_context():
            ticket = ticket_factory(subject="Individual Issue", author_id=user.id, team_id=None)

            create_ticket_message(text="Let me look into this", author_id=admin.id, ticket=ticket, is_admin=True)
    ...
    def test_user_reply_to_assigned_ticket_notifies_assigned_admin(
        ...
        """
        with app.app_context():
            ticket = ticket_factory(subject="Assigned Issue", author_id=user.id)
            ticket.assign_to_user(admin.id, commit=True)

            create_ticket_message(text="Thanks, here's more info", author_id=user.id, ticket=ticket, is_admin=False)
    ...
    def test_user_reply_to_unassigned_ticket_goes_to_admin_inbox(
        self, app, db_session, user, ticket_factory, email_config, mock_ses
    ):
        """
        ...
        with app.app_context():
            ...
            ticket = ticket_factory(subject="Unassigned Issue", author_id=user.id)

            create_ticket_message(text="Any update on this?", author_id=user.id, ticket=ticket, is_admin=False)
    ...
    def test_implicit_assignment_on_admin_reply(
        ...
        with app.app_context():
            ...
            create_ticket_message(text="I'll handle this", author_id=admin.id, ticket=ticket, is_admin=True)

            db_session.refresh(ticket)

            assert ticket.assigned_to == admin.id

    def test_email_not_sent_when_not_configured(self, app, db_session, user, event, team_factory):
        """
        ...
        with app.app_context():
            ...
            with patch("ng.emails.services.email_sender.logger") as mock_logger:
                create_ticket(
                    subject="Test Issue", text="I need help with something", current_user=user, event_id=event.id
                )
    ...
    def test_multiple_admin_emails_for_new_tickets(self, app, db_session, user, event, team_factory, mock_ses):
        """
        ...
        with app.app_context():
            ...
            app.config["AWS_DEFAULT_REGION"] = "us-east-1"
            app.config["AWS_SES_FROM_EMAIL"] = "noreply@ctf.com"
            app.config["ADMIN_SUPPORT_INBOX_EMAILS"] = "admin1@ctf.com, admin2@ctf.com ,admin3@ctf.com"
            app.config["SERVER_DOMAIN"] = "http://localhost:8000"
            ...
            expected_emails = {"admin1@ctf.com", "admin2@ctf.com", "admin3@ctf.com"}
            recipient_emails = call_args["Destination"]["ToAddresses"]
            assert set(recipient_emails) == expected_emails

    def test_email_service_graceful_failure(self, app, db_session, user, event, team_factory, email_config):
        """
    ...
    def test_no_emails_sent_when_no_recipients_found(
        ...
        with app.app_context():
            ...
            ticket = ticket_factory(subject="Orphaned Ticket", author_id=user.id)

            create_ticket_message(text="Any update?", author_id=user.id, ticket=ticket, is_admin=False)
