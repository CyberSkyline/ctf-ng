...
class TestScoringIntegrations:
    ...
    """
    @patch('ng.scoring.controllers.user_actions.submit_answer.NotificationService')
    ...
    @patch('ng.scoring.controllers.user_actions.redeem_hint.NotificationService')
...
class TestEventIntegrations:
    ...
    """
    @patch('ng.event.controllers.admin.manage_event_lifecycle.NotificationService')
...
class TestIntegrationWithRealService:
    ...
    def test_ticket_reply_creates_stored_notification_and_websocket(
        ...
        with patch('ng.notifications.services.notification_service.emit_event'
            ...
            assert refetch_call[1]['data']['path'
                                           ] == f"/ng/support/tickets/{ticket.id}"
    ...
    def test_attempt_submission_only_websocket_no_stored(
        ...
        with patch('ng.notifications.services.notification_service.emit_event'
            ...
            challenge_call = next(
                (
                    c for c in calls if
                    f"/ng/events/{event.id}/challenges/{challenge.id}" in str(c)
                ),
                None
            )
