...
SELECT ng_demographics.event_id, ng_events.name AS event_name, COUNT(DISTINCT ng_demographics.user_id) AS user_count, COUNT(DISTINCT ng_teams.id) AS team_count
...
GROUP BY ng_demographics.event_id, ng_events.name;
...
SELECT ng_teams.event_id, ng_events.name AS event_name, COUNT(DISTINCT ng_teams.id)
FROM ng_teams
